...
<style>.hidden-ing { display: none; }</style>
        ...
        <div class="content-card">
            ...
            <div id="optionsList">
                @if(count($options) > 0)
                    ...
                    <div style="background:#f9f9f9;border-radius:8px;margin-bottom:0.4rem;border:2px solid transparent;padding:0.5rem 0.75rem;" id="opt-{{ $option->id }}">
                        <div style="display:flex;align-items:center;justify-content:space-between;">
                            ...
                            <div style="display:flex;align-items:center;gap:0.4rem;">
                                <button type="button" onclick="document.getElementById('ing-{{ $option->id }}').classList.toggle('hidden-ing')" style="background:#fff;color:#C0392B;border:1px solid #f4b1a0;border-radius:6px;padding:0.25rem 0.5rem;font-size:0.72rem;cursor:pointer;">
                                    <i class="bi bi-box"></i> {{ $option->ingredients->count() }}
                                </button>
                        ...
                        <div id="ing-{{ $option->id }}" class="hidden-ing" style="margin-top:0.5rem;padding-top:0.5rem;border-top:1px dashed #ddd;">
                            <p style="font-size:0.7rem;color:#888;margin:0 0 0.4rem;">Ingredients deducted only when this option is selected by the customer.</p>
                            @if($option->ingredients->count() > 0)
                                @foreach($option->ingredients as $ing)
                                <div style="display:flex;justify-content:space-between;align-items:center;background:#fff;border-radius:6px;padding:0.3rem 0.5rem;font-size:0.75rem;margin-bottom:0.25rem;">
                                    <span>{{ $ing->inventory->item_name ?? '(removed)' }} — {{ rtrim(rtrim(number_format($ing->quantity_used, 3), '0'), '.') }} {{ $ing->inventory->unit ?? '' }}</span>
                                    <form action="{{ route('admin.menu-options.ingredients.delete', [$option->id, $ing->id]) }}" method="POST" style="margin:0;" onsubmit="return confirm('Remove this ingredient?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" style="background:none;color:#C0392B;border:none;cursor:pointer;font-size:0.75rem;">
                                ...
                                @endforeach
                            @endif
                            <form action="{{ route('admin.menu-options.ingredients.add', $option->id) }}" method="POST" style="display:flex;gap:0.4rem;align-items:flex-end;flex-wrap:wrap;margin-top:0.35rem;">
                                @csrf
                                <select name="inventory_id" required class="form-control-custom" style="margin-bottom:0;flex:1;min-width:140px;font-size:0.75rem;padding:0.3rem 0.5rem;">
                                    <option value="">-- Inventory ingredient --</option>
                                    @foreach($inventoryItems as $invItem)
                                        <option value="{{ $invItem->id }}">{{ $invItem->item_name }} ({{ $invItem->unit }})</option>
                                    @endforeach
                                </select>
                                <input type="number" name="quantity_used" placeholder="qty" step="0.001" min="0.001" required class="form-control-custom" style="margin-bottom:0;width:80px;font-size:0.75rem;padding:0.3rem 0.5rem;">
                                <button type="submit" class="btn-primary-custom" style="padding:0.3rem 0.65rem;font-size:0.72rem;">
                                    <i class="bi bi-plus"></i> Add
                                </button>
                            </form>
                        </div>
...
function saveAssignment() {
    ...
    fetch('{{ url('admin/menu-options/assign') }}/' + selectedItemId, {
        method: 'POST',
        headers: {
