...
<div id="itemModal" style="display:none; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.5); z-index:999; align-items:center; justify-content:center; padding:1rem;">
    <div style="background:white; border-radius:12px; padding:1.5rem; max-width:600px; width:100%; max-height:90vh; overflow-y:auto;">
        ...
        <form id="itemForm" action="{{ route('admin.new-menu-item.post') }}" method="POST" enctype="multipart/form-data">
            ...
            <details style="margin-bottom: 0.85rem; background:#f4f4f4; border-radius:8px; padding:0.55rem 0.75rem;">
                ...
                <p style="font-size:0.7rem; color:#888; margin:0.5rem 0;">
                    Leave empty for new items — use the <strong>Recipe Ingredients</strong> section below instead.
                </p>
        ...
        {{-- =========== Recipe Ingredients (multi-ingredient) =========== --}}
        {{-- Rendered outside the main form so the sub-forms below do not nest. --}}
        <div style="margin-top:1.25rem; border:2px solid #F4845F; border-radius:10px; padding:0.85rem;">
            <p style="font-size:0.95rem; font-weight:700; color:#C0392B; margin:0 0 0.25rem;">
            ...
            <p style="font-size:0.72rem; color:#555; margin:0 0 0.4rem;">
                Use this section for multiple ingredients. These are always deducted when this menu item is ordered.
                Example: Pizza Dough 1 pc, Cheese 50 g, Tomato Sauce 30 ml, Pepperoni 30 g.
            </p>
            <p style="font-size:0.7rem; color:#888; margin:0 0 0.6rem;">
                <i class="bi bi-info-circle"></i>
                The unit comes from the selected inventory item. To use grams or ml, edit the inventory item's unit first.
            </p>

            {{-- Add-mode notice --}}
            <div id="recipeAddNotice" style="background:#fff3cd; color:#7a5c00; padding:0.5rem 0.75rem; border-radius:6px; font-size:0.75rem; display:none;">
                <i class="bi bi-exclamation-triangle"></i> Save the menu item first, then re-open it to add recipe ingredients.
            </div>

            {{-- One block per existing menu item. JS shows the matching one. --}}
            @if(isset($menuItems))
                @foreach($menuItems as $mi)
                    @php $recipe = $mi->recipeIngredients; @endphp
                    <div class="recipe-block" id="recipe-{{ $mi->id }}" style="display:none;">

                        @if($recipe->isEmpty())
                            <div style="background:#fff8e1; color:#7a5c00; padding:0.5rem 0.75rem; border-radius:6px; font-size:0.75rem; margin-bottom:0.6rem;">
                                <i class="bi bi-exclamation-triangle"></i> No recipe ingredients assigned yet.
                            </div>
                        @else
                            <table style="width:100%; font-size:0.78rem; margin-bottom:0.6rem;">
                                <thead>
                                ...
                                <tbody>
                                    ...
                                    <tr style="border-top:1px solid #f0f0f0;">
                                        ...
                                        <td style="padding:0.35rem 0.4rem; text-align:right;">
                                            <form action="{{ route('admin.menu-items.ingredients.delete', [$mi->id, $row->id]) }}" method="POST" style="margin:0; display:inline;" onsubmit="return confirm('Remove this ingredient?')">
                                                @csrf @method('DELETE')
                        ...
                        <form action="{{ route('admin.menu-items.ingredients.add', $mi->id) }}" method="POST" style="display:flex; gap:0.4rem; align-items:flex-end; flex-wrap:wrap;">
                            @csrf
                            <div style="flex:2; min-width:170px;">
                                <label class="form-label-custom">Ingredient</label>
                                <select name="inventory_id" required class="form-control-custom recipe-ing-select" data-block="{{ $mi->id }}" style="margin-bottom:0;">
                                    <option value="">-- Select --</option>
                                    @if(isset($inventoryItems))
                                        @foreach($inventoryItems as $invItem)
                                            <option value="{{ $invItem->id }}" data-unit="{{ $invItem->unit }}">
                            ...
                            <div style="flex:1; min-width:120px;">
                                <label class="form-label-custom">
                                    Quantity Used <span class="recipe-unit-label" style="color:#888; font-weight:500;"></span>
                                </label>
                                <input type="number" name="quantity_used" class="form-control-custom" step="0.001" min="0.001" required style="margin-bottom:0;">
                            </div>
                            <button type="submit" class="btn-primary-custom" style="padding:0.5rem 0.9rem;">
...
<script>
    ...
    function openAddModal() {
        ...
        showRecipeBlock(null); // hide all blocks, show "save first" notice
    ...
    // Show only the recipe block for the given menu item id. Pass null in add mode.
    function showRecipeBlock(id) {
        document.querySelectorAll('.recipe-block').forEach(function (b) { b.style.display = 'none'; });
        var notice = document.getElementById('recipeAddNotice');
        if (id) {
    ...
    // Live unit label next to the Quantity field — pulled from the chosen inventory option.
    document.addEventListener('change', function (e) {
        if (!e.target.classList || !e.target.classList.contains('recipe-ing-select')) return;
        var opt = e.target.options[e.target.selectedIndex];
        var unit = opt ? (opt.dataset.unit || '') : '';
        var block = document.getElementById('recipe-' + e.target.dataset.block);
        if (!block) return;
        var label = block.querySelector('.recipe-unit-label');
    ...
    // After adding/removing a recipe ingredient the controller redirects back here
    // with a session flag — re-open the same item's modal so the user lands where
    // they were, not in the middle of a half-rendered page.
    var reopenForId = "{{ session('open_recipe_for') }}";
    if (reopenForId) {
        document.addEventListener('DOMContentLoaded', function () {
            var btn = document.querySelector('button.btn-edit-custom[data-id="' + reopenForId + '"]');
            if (btn) {
                openEditModal(btn);
                var block = document.getElementById('recipe-' + reopenForId);
