            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f0f0f0;">
                    ...
                    <td style="padding:0.6rem;text-align:center;">
                        <span style="background:{{ $branch->is_active ? '#4CAF50' : '#888' }};color:white;padding:0.15rem 0.6rem;border-radius:20px;font-size:0.7rem;font-weight:600;">
                    ...
                    <td style="padding:0.6rem;text-align:center;white-space:nowrap;">
                        <form action="{{ route('admin.branches.toggle', $branch->id) }}" method="POST"
                            ...
                            onsubmit="return confirm('{{ $branch->is_active ? 'Close ' . $branch->name . '? Customers will no longer be able to order from this branch.' : 'Open ' . $branch->name . ' for orders?' }}')">
                            @csrf @method('PUT')
                            <button type="submit"
                                title="{{ $branch->is_active ? 'Stop accepting orders' : 'Start accepting orders' }}"
                                style="background:{{ $branch->is_active ? '#F4845F' : '#4CAF50' }};color:white;border:none;border-radius:6px;padding:0.3rem 0.6rem;font-size:0.72rem;cursor:pointer;font-weight:600;">
                                <i class="bi {{ $branch->is_active ? 'bi-door-closed' : 'bi-door-open' }}"></i>
                                {{ $branch->is_active ? 'Set as Closed' : 'Set as Open' }}
                            </button>
                        </form>
                        ...
                        @else
                        <span style="color:#aaa;font-size:0.72rem;margin-left:0.4rem;">Main branch</span>
                        @endif
                    </td>
                </tr>
                @endforeach
