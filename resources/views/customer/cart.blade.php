...
<body>
    <div class="app-wrapper">
        ...
        <div class="content-section">
            ...
            <div class="cart-item">
                <div class="img-wrapper">
                    @if(!empty($item['image']))<img src="{{ \App\Support\Img::url($item['image']) }}" alt="{{ $item['name'] }}">
        ...
        <div class="bottom-fixed">
            <form action="{{ route('customer.place-order') }}" method="POST" id="mainOrderForm">
                ...
                @php $idx = 0; @endphp
                @foreach($cart as $itemId => $item)
                {{-- cart_key uniquely identifies this cart line (menu item + selected options) --}}
                {{-- so the controller doesn't collapse two lines of the same item that have different options. --}}
                <input type="hidden" name="items[{{ $idx }}][cart_key]" value="{{ $itemId }}">
                ...
                <div style="margin-top:0.5rem;margin-bottom:0.2rem;">
                    <p style="font-size:0.75rem;font-weight:600;color:#333;margin:0 0 0.3rem;">Payment Method</p>
                    <div style="display:flex;gap:0.4rem;">
                        <button type="button" id="payCashBtn" class="pay-opt active" onclick="selectPayment('cash')">
                            <i class="bi bi-cash-coin"></i> Cash
                        </button>
                        <button type="button" id="payGcashBtn" class="pay-opt" onclick="selectPayment('gcash')">
                            <i class="bi bi-phone"></i> GCash
                        </button>
                    </div>
                    <p id="gcashNote" style="display:none;font-size:0.68rem;color:#856404;background:#fff3cd;border-radius:6px;padding:0.3rem 0.5rem;margin-top:0.3rem;">
                        <i class="bi bi-info-circle"></i> GCash payment will be verified by staff.
                    </p>
                </div>
    ...
    <script>
        ...
        function applyVoucher() {
            ...
            fetch('{{ route('customer.apply-voucher') }}', {
                    method: 'POST',
                    headers: {
        ...
        function selectPayment(method) {
            ...
            document.getElementById('payGcashBtn').classList.toggle('active', method === 'gcash');
            document.getElementById('gcashNote').style.display = method === 'gcash' ? 'block' : 'none';
        }
    </script>
